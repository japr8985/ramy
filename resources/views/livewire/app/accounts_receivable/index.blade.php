<div class="p-margin-mobile md:p-gutter space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            ...
            <p class="text-body-md text-on-surface-variant">Gestione los saldos pendientes de sus clientes y el registro
                de cobranzas de manera centralizada.</p>
        </div>
        <div class="relative w-full md:max-w-xs">
            <span
                class="absolute left-3 top-1/2 -translate-y-1/2 material-symbols-outlined text-outline text-sm">search</span>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div class="bg-surface-container-lowest p-5 rounded-xl border border-outline-variant shadow-xs">
            ...
            <h3 class="text-headline-lg font-bold text-primary">${{ number_format($stats['total_receivables'], 2) }}
            </h3>
            <p class="text-xs text-outline mt-1">Ref: Bs.
                {{ number_format($stats['total_receivables'] * $exchangeRate, 2, ',', '.') }}</p>
        </div>
        <div class="bg-surface-container-lowest p-5 rounded-xl border border-outline-variant shadow-xs">
            ...
            <table class="w-full min-w-[900px] text-left border-collapse">
                <thead>
                    <tr class="bg-surface-container-low text-label-caps text-on-surface-variant select-none text-xs">
                        ...
                        <th class="px-6 py-4 font-semibold border-b border-outline-variant text-center">Fecha Emisión
                        </th>
                        <th class="px-6 py-4 font-semibold border-b border-outline-variant text-right">Total ($)</th>
                        <th class="px-6 py-4 font-semibold border-b border-outline-variant text-right">Monto Abonado
                        </th>
                        <th class="px-6 py-4 font-semibold border-b border-outline-variant text-right">Saldo Deudor ($)
                        </th>
                        <th class="px-6 py-4 font-semibold border-b border-outline-variant text-right">Saldo Deudor
                            (Bs.)</th>
                ...
                <tbody class="text-body-sm divide-y divide-outline-variant">
                        ...
                        <tr class="hover:bg-surface-container-low/30 transition-colors">
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    <div
                                        class="w-8 h-8 rounded bg-surface-container flex items-center justify-center text-outline">
                                        <span class="material-symbols-outlined text-lg">person</span>
                                    </div>
                                    <div>
                                        <p class="font-bold text-primary">
                                            {{ $invoice->customer?->name ?? 'Cliente General' }}</p>
                                        <p class="text-xs text-on-surface-variant font-data-mono">
                                            {{ $invoice->sale?->invoice_number }}</p>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 text-center font-data-mono text-xs">
                                {{ \Carbon\Carbon::parse($invoice->sale_date)->format('Y-m-d') }}</td>
                            <td class="px-6 py-4 text-right font-data-mono">${{ number_format($invoice->total, 2) }}</td>
                            <td class="px-6 py-4 text-right font-data-mono text-success">
                                ${{ number_format($invoice->paid_amount, 2) }}</td>
                            <td class="px-6 py-4 text-right font-bold font-data-mono text-primary">
                                ${{ number_format($invoice->balance, 2) }}</td>
                            <td class="px-6 py-4 text-right font-bold font-data-mono text-secondary">Bs.
                                {{ number_format($invoice->balance_ves, 2, ',', '.') }}</td>
                            <td class="px-6 py-4 text-center">
                                @if(\Carbon\Carbon::parse($invoice->due_date)->isPast())
                                    <span
                                        class="inline-flex items-center px-2 py-0.5 bg-error-container text-on-error-container rounded text-xs font-bold gap-1 mx-auto w-max">
                                        <span class="material-symbols-outlined text-xs">event_busy</span> Vencido
                                    </span>
                                @else
                                    <span
                                        class="inline-flex items-center px-2 py-0.5 bg-surface-container-highest text-primary rounded text-xs font-bold gap-1 mx-auto w-max">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center justify-center gap-3">
                                    ...
                                    <button wire:click="openHistoryModal('{{ $invoice->id }}')"
                                        class="text-outline hover:text-primary font-medium text-xs inline-flex items-center gap-0.5 hover:underline"
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="px-6 py-10 text-center text-on-surface-variant text-body-sm">No hay
                                cuentas de crédito pendientes por cobrar.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <div class="{{ $modal ? '' : 'hidden' }} fixed inset-0 z-50 flex items-end md:items-center justify-center p-4">
        ...
        <div
            class="relative bg-surface-container-lowest w-full max-w-md rounded-t-2xl md:rounded-2xl shadow-2xl overflow-hidden border border-outline-variant flex flex-col transform animate-in slide-in-from-bottom duration-200 z-10">
            <div class="p-6 border-b border-outline-variant flex justify-between items-center bg-surface-container-low">
                ...
                <button class="text-on-surface-variant hover:text-error transition-colors" type="button"
            ...
            <form wire:submit.prevent="postPayment" class="p-6 space-y-4" x-data="{
            ...
            this.usd = this.rate > 0 ? parseFloat((parsed / this.rate).toFixed(4)) : 0;
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <div
                            class="text-xs font-bold font-data-mono bg-surface-container p-2 rounded border border-outline-variant text-primary">
                            ${{ number_format($current_invoice_balance, 2) }}<br>
                        </div>
                        <span class="text-[10px] font-normal text-secondary">Bs.
                            {{ number_format($current_invoice_balance * $exchangeRate, 2, ',', '.') }}</span>
                    </div>
                    <div>
                        <label class="block text-[11px] font-bold text-outline uppercase mb-1">Fecha de
                            Recepción</label>
                        <input type="date" wire:model="payment_date"
                            class="w-full bg-surface border-outline-variant rounded-lg text-body-sm focus:ring-secondary py-1.5" />
                    </div>
                </div>

                <!-- SWITCH / CONMUTADOR DE MONEDA DE ENTRADA -->
                <div class="space-y-2">
                    <label class="block text-[11px] font-bold text-outline uppercase">Moneda de Recepción en
                        Caja</label>
                    <div class="grid grid-cols-2 bg-surface border border-outline-variant p-1 rounded-xl">
                        <button type="button" @click="currency = 'USD'"
                            :class="currency === 'USD' ? 'bg-primary text-on-primary shadow-xs' : 'text-on-surface-variant hover:text-primary'"
                            class="py-1.5 rounded-lg text-xs font-bold transition-all">
                            DÓLARES ($)
                        </button>
                        <button type="button" @click="currency = 'VES'"
                            :class="currency === 'VES' ? 'bg-secondary text-on-secondary shadow-xs' : 'text-on-surface-variant hover:text-primary'"
                            class="py-1.5 rounded-lg text-xs font-bold transition-all">
                            BOLÍVARES (Bs.)
                        </button>
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="space-y-1">
                        ...
                        <div class="relative">
                            <span
                                class="absolute inset-y-0 left-0 pl-3 flex items-center text-on-surface-variant font-bold text-xs"
                                :class="currency !== 'USD' && 'opacity-40'">$</span>
                            <input type="number" step="0.0001" x-model="usd" @input="calculateFromUsd()"
                                :readonly="currency !== 'USD'" :disabled="currency !== 'USD'"
                    ...
                    <!-- Input en Bolívares -->
                    <div class="space-y-1">
                        <label class="block text-[11px] font-bold text-outline uppercase">Monto en Bolívares
                            (Bs.)</label>
                        <div class="relative">
                            <span
                                class="absolute inset-y-0 left-0 pl-3 flex items-center text-on-surface-variant font-bold text-xs"
                                :class="currency !== 'VES' && 'opacity-40'">Bs.</span>
                            <input type="number" step="0.01" x-model="ves" @input="calculateFromVes()"
                                :readonly="currency !== 'VES'" :disabled="currency !== 'VES'"
                ...
                <div
                    class="p-3 bg-surface border border-outline-variant/60 rounded-xl flex items-center justify-between text-[11px] text-on-surface-variant font-medium">
                    <span class="flex items-center gap-1"><span
                            class="material-symbols-outlined text-xs text-outline">info</span> Tasa BCV Oficial</span>
                    <span class="font-data-mono font-bold text-primary">Bs.
                        {{ number_format($exchangeRate, 2, ',', '.') }}</span>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="space-y-1">
                        ...
                        <select wire:model="payment_method"
                            class="w-full bg-surface border-outline-variant rounded-lg text-body-sm py-2">
                ...
                <div class="pt-4 border-t border-outline-variant/40 flex justify-end gap-3">
                    <button type="button"
                        class="px-5 py-2 border border-outline-variant bg-white text-on-surface font-semibold rounded-lg hover:bg-surface-container transition-all text-xs"
                        wire:click="closePaymentModal">Cancelar</button>
                    <button type="submit"
                        class="px-6 py-2 bg-primary text-on-primary font-bold rounded-lg hover:opacity-90 transition-all shadow-md text-xs">Registrar
                        Cobro</button>
                </div>
            </form>
        </div>
    ...
    <div class="{{ $historyModal ? '' : 'hidden' }} fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div
            class="relative bg-surface-container-lowest w-full max-w-lg rounded-2xl border border-outline-variant shadow-2xl flex flex-col transform animate-in zoom-in duration-150 z-10 max-h-[85vh]">
            <div
            ...
            <div class="p-6 overflow-y-auto space-y-3 flex-1 custom-scrollbar">
                @forelse($paymentHistory as $pay)
                    <div
                        class="border border-outline-variant/60 rounded-xl p-3 bg-surface flex items-center justify-between gap-4">
                        <div class="space-y-1">
                            <span
                                class="text-[10px] px-2 py-0.5 bg-success/10 text-success rounded-full font-bold uppercase">
                                {{ $pay['payment_method'] === 'Bank Transfer' ? 'Pago Móvil' : ($pay['payment_method'] === 'Cash' ? 'Efectivo' : 'Punto') }}
                            </span>
                            <p class="text-xs font-data-mono text-outline">
                                {{ \Carbon\Carbon::parse($pay['payment_date'])->format('Y-m-d H:i') }}</p>
                            @if($pay['notes'])
                                <p class="text-[11px] text-primary italic font-medium">Ref: {{ $pay['notes'] }}</p>
                            @endif
                        </div>
                        <div class="text-right">
                            <p class="font-bold font-data-mono text-primary text-sm">
                                ${{ number_format($pay['amount_usd'], 2) }}</p>
                            <p class="text-[10px] font-semibold text-secondary">A tasa:
                                {{ number_format($pay['exchange_rate'], 2, ',', '.') }} Bs.</p>
                        </div>
                    </div>
                @empty
            ...
            <div class="p-4 bg-surface-container-low border-t border-outline-variant flex justify-end">
                <button type="button" wire:click="closeHistoryModal"
                    class="px-5 py-1.5 bg-primary text-on-primary font-bold text-xs rounded-xl shadow-xs hover:opacity-90">
                    Cerrar Auditoría
                </button>
            </div>
        </div>
